...
import { validateIngestionConfig } from "./config-validation";

export function createIngestionClient() {
  const { url, key } = validateIngestionConfig(process.env.NEXT_PUBLIC_SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
...
export async function syncProvider(options: SyncProviderOptions, client: SupabaseClient<Database> = createIngestionClient()): Promise<SyncResult> {
  ...
  const maxRequests = source === "SAM" ? 1 : options.maxRequests ?? providerMaxRequests(source);
  ...
  try {
    for (let page = 1; page <= maxRequests; page += 1) {
      ...
      if (rateLimited) {
        await updateSyncState(client, source, { upstream_http_status: 429, last_error_type: "rate_limited", last_safe_error: "SAM.gov rate limit reached.", records_fetched: recordsFetched, records_inserted: recordsInserted, records_updated: recordsUpdated, is_stale: true });
    ...
    await updateSyncState(client, source, { ...(source === "SAM" ? { upstream_http_status: 200 } : {}), last_success_at: new Date().toISOString(), last_error_type: null, last_safe_error: null, records_fetched: recordsFetched, records_inserted: recordsInserted, records_updated: recordsUpdated, is_stale: false });
    ...
    await updateSyncState(client, source, { ...(source === "SAM" ? { upstream_http_status: diagnostic?.upstreamStatus ?? null } : {}), last_error_type: diagnostic?.errorType ?? "sync_failed", last_safe_error: diagnostic?.safeErrorMessage ?? safeError, records_fetched: recordsFetched, records_inserted: recordsInserted, records_updated: recordsUpdated, is_stale: true });
