...
export function SiteFooter({ authenticated = false }: { authenticated?: boolean }) {
  return <footer className={`site-footer${authenticated ? " authenticated" : ""}`}><div><strong>International Procurement Office</strong><nav><Link href="/contact">Contact</Link><Link href="/faq">FAQ</Link><Link href="/privacy">Privacy</Link><Link href="/terms">Terms</Link>{authenticated ? <Link href="/settings">Settings</Link> : null}</nav></div><LanguageSelector /><a href="mailto:intelligence@internationalprocurementoffice.com">intelligence@internationalprocurementoffice.com</a><p>International Procurement Office is an independent procurement intelligence platform and is not affiliated with or endorsed by any government agency.</p></footer>;
}
